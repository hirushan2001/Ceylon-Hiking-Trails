...
import { X, CheckCircle2, Mountain, MapPin } from 'lucide-react';
...
export const AdminTrailModal: React.FC<AdminTrailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-slate-900 rounded-3xl shadow-2xl border border-slate-800 p-6 sm:p-8 overflow-y-auto text-white">
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2 text-slate-400 hover:text-white rounded-full"
        >
          <X size={20} />
        </button>
        ...
        {successMsg ? (
          <div className="py-12 text-center space-y-3">
            ...
            <p className="text-sm text-slate-400">Saved directly to backend REST API & Neon PostgreSQL database.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="flex items-center gap-2.5 border-b border-slate-800 pb-4">
              <div className="p-2.5 rounded-xl bg-[#5C5CFF]/20 text-[#5C5CFF]">
                <Mountain size={22} />
              </div>
              <div>
                ...
                <p className="text-xs text-slate-400">Backend Express API CRUD Form</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 rounded-xl border border-slate-800 bg-slate-800/80 text-white text-xs font-medium focus:ring-2 focus:ring-[#5C5CFF]"
                />
              </div>
              <div>
                <label className="text-xs font-bold uppercase text-slate-400 block mb-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 rounded-xl border border-slate-800 bg-slate-800/80 text-white text-xs font-medium"
                />
              </div>
              <div>
                <label className="text-xs font-bold uppercase text-slate-400 block mb-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-400 block mb-1">Distance (km)</label>
                <input
                  type="number"
                  step="0.1"
                  value={distanceKm}
                  onChange={(e) => setDistanceKm(Number(e.target.value))}
                  className="w-full p-2.5 rounded-xl border border-slate-800 bg-slate-800/80 text-white text-xs font-medium"
                />
              </div>
              <div>
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-400 block mb-1">Elevation Gain (m)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-400 block mb-1">Highest Peak (m)</label>
                <input
                  type="number"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-400 block mb-1">Hero Image URL</label>
                <input
                  type="text"
                  value={heroImage}
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                required
                className="w-full p-3 rounded-xl border border-slate-800 bg-slate-800/80 text-white text-xs focus:ring-2 focus:ring-[#5C5CFF]"
              />
            </div>
