...
export const TrailCard: React.FC<TrailCardProps> = ({ trail, compact = false }) => {
  ...
  return (
    <div className="group relative rounded-3xl overflow-hidden bg-white border border-[#E7E0D8] shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full">
      {/* Image Header */}
      <div className="relative w-full h-52 sm:h-56 overflow-hidden bg-[#F3ECE4]">
        <Image
          src={trail.heroImage}
          alt={trail.name}
          fill
      ...
      <div className="p-5 flex flex-col flex-grow justify-between">
        <div>
          <div className="flex items-center justify-between gap-2 mb-2">
            <Link href={`/trails/${trail.slug}`} className="focus:outline-none">
              <h3 className="font-bold text-lg text-[#1C1917] group-hover:text-[#2D6A4F] transition-colors line-clamp-1 font-heading">
                {trail.name}
              </h3>
            </Link>
          ...
          <p className="text-xs sm:text-sm text-[#78716C] line-clamp-2 mb-4">
        ...
        <div>
          {/* Key Stats Bar */}
          <div className="grid grid-cols-3 gap-2 py-3 px-3 rounded-2xl bg-[#E8F5E9]/60 text-[#1C1917] text-xs mb-4">
            <div className="flex flex-col items-center justify-center text-center border-r border-[#E2E8F0] pr-1">
              <span className="text-[#78716C] mb-0.5">
                <Navigation size={13} />
              </span>
              <span className="font-bold text-[#1C1917]">{trail.distanceKm} km</span>
            ...
            <div className="flex flex-col items-center justify-center text-center border-r border-[#E2E8F0] pr-1">
              ...
              <span className="font-bold text-[#1C1917]">{trail.estimatedDuration.split(' ')[0]} h</span>
            ...
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <span className="font-bold text-[#1C1917]">+{trail.elevationGainMeters} m</span>
          ...
          <Link
            ...
            className="w-full inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-full font-bold text-xs bg-[#2D6A4F]/10 text-[#2D6A4F] hover:bg-[#2D6A4F] hover:text-white transition-all duration-200"
          >
